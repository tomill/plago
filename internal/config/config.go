...
type Config struct {
	...
	DiscordChannelIDs  List       `arg:"--,env:DISCORD_CHANNELS"   help:"Used when --in discord  A newline/space/comma separated list (after # in line is ignored)"`
	DiscordChannels    Sheet[struct {
		ChannelID   string
		ChannelName string
	}] `arg:"--,env:DISCORD_CHANNELS_SHEET"                        help:"Used when --in discord  Spreadsheet URL containing the values. Used when DiscordChannelIDs is empty"`
	...
	SlackChannels         Sheet[struct {
		ChannelID   string
		ChannelName string
	}] `arg:"--,env:SLACK_CHANNELS_SHEET"                          help:"Used when --in slack    Spreadsheet URL containing the values. Used when SlackChannelIDs is empty"`
	...
	YoutubeChannels   Sheet[struct {
		ChannelID   string
		ChannelName string
	}] `arg:"--,env:YOUTUBE_CHANNELS_SHEET"                        help:"Used when --in youtube  Spreadsheet URL containing the values. Used when YoutubeChannelIDs is empty"`
	...
	SheetCredentials  string       `arg:"--,env:SHEET_CREDENTIALS" help:"Sheets API credentials JSON. Used with the _SHEET setting"`
}
